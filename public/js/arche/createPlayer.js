...
POLY2DCOL_RECT.rules.layer = CollisionLayer.PLAYER;
POLY2DCOL_RECT.rules.mask = CollisionLayer.DEFAULT | CollisionLayer.ENEMY;
  ...
  const sys = {
    ...
    update: (ginfo) => {
      ...
      if (inputSys.keys.pressed.has('s'))
        t2d.pos.add(Vector2d.fromRadians(t2d.rot).negate().mul(dtSec).mul(250));
